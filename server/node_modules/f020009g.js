	...
	return new Promise(function (resolve, reject) {
		...
		if (obj.fullsync === true) {
			...
			q = `SELECT
					...
					lig_active,
					extract(epoch from lig_mtime)::integer as lig_mtime
				from trade.link_Item_Group lig
				JOIN (
					SELECT MAX(st2.st_stime) AS st_stime
					FROM trade.sync_token st2
					WHERE st2.t_id = $1 AND st2.st_table = 'f020009g' AND st2.st_result = TRUE
				) AS st ON (st.st_stime <= lig.lig_mtime);`;
		}
		db.query(q, [client.t_id]).then(
			(value) => {
				db.one(`INSERT INTO trade.sync_token (t_id, st_table, st_stime, st_etime, st_result) VALUES
  						($1,'f020009g',now(),null, FALSE ) RETURNING trade.sync_token.st_id`, [client.t_id]).then(
					(st) => {
						var sync = +st.st_id;
