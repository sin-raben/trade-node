...
exports.fun = function(client, obj, db) {
	return new Promise(function(resolve, reject) {
		...
		if (obj.fullsync === true) {
			q = `SELECT n.*,na.na_action,na_data
				FROM trade.news n
				JOIN trade.links_news ln ON (n.n_id = ln.n_id) AND (ln.at_id = 4)
				JOIN trade.token t ON (ln.any_id = t.u_id)
				LEFT JOIN trade.news_action na ON (t.u_id = na.u_id) AND (na.n_id = n.n_id)
				WHERE (t.t_id = $1);`;
		} else {
			q = `SELECT n.*,na.na_action,na_data
				FROM trade.news n
		...
		db.query(q, [client.t_id]).then(value => {
			db
				.one(
					`INSERT INTO trade.sync_token (t_id, st_table, st_stime, st_etime, st_result) VALUES
  						($1,'f020018g',now(),null, FALSE ) RETURNING trade.sync_token.st_id`,
					[client.t_id]
				)
				.then(st_id => {
